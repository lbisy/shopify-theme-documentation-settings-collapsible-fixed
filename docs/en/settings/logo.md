...
The **Logo** settings manage the store's brand logo and browser icon.
...
- Use a transparent PNG if the logo appears on different background colors.
- Upload a high-resolution image so it remains sharp on high-density displays.
- Remove unnecessary blank space around the artwork.

::: tip
After uploading, check the logo on desktop and mobile.
:::
...
- The favicon has updated.

::: info
Browsers can cache the previous favicon. If the new icon does not appear immediately, clear the browser cache or check in a private window.
:::
