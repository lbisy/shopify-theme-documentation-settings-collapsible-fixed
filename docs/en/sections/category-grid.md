# Category Grid

Use this section to add **Category Grid** content to a page.

## How to add the section

1. Open **Shopify Admin → Online Store → Themes**.
2. Click **Customize** on the theme.
3. Open the required page or template.
4. Click **Add section**.
5. Select **Category Grid**.
6. Configure the content and click **Save**.

**Main settings:** Category Grid, Category Grid, Category Grid, Category Grid, Category Grid, Category Grid, Category Grid, Category Grid, Category Grid, Category Grid, Category Grid, Category Grid, Category Grid, Category Grid, Category Grid

**Available blocks:** Category Grid

## Recommended workflow

Preview the section on desktop and mobile. Confirm image cropping, spacing, button links, text contrast, and content order before publishing.

![Category Grid settings and storefront preview](/images/en/category-grid.svg)
